...
for(i=1;i<=sourceImages.len();i++) {
	...
	try {
		...
		fileName = getFileFromPath(sourceImages[i]);
		newFilename = slugify(result.candidates[1].content.parts[1].text.trim()) & "." & listLast(fileName, ".");
		writeoutput("<p>Will save as #newFilename#</p>");
	} catch(any e) {
